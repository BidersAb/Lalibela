    ...
    var NivoSlider = function(element, options){
        ...
        if(settings.controlNav){
			if(settings.controlNav===true){
				...
				for(var i = 0; i < kids.length; i++){
					if(settings.controlNavThumbs){
						...
						if (settings.controlNavThumbsFromRel) {
							...
							nivoControl.append('<a class="nivo-control" rel="'+ i +'"><img src="'+ child.attr('src') +'" alt="" /></a>');
						}
					} else {
				...
				var nivoControl=settings.controlNav;
			}
            //Set initial active link
            $(nivoControl).find('a:eq('+ vars.currentSlide +')').addClass('active');
            
            $(nivoControl).find('a').click(function(){
                if(vars.running) return false;
		...
		var nivoRun = function(slider, kids, settings, nudge){
			...
			//Set active links
			if(settings.controlNav){
				$(nivoControl).find('a').removeClass('active');
				$(nivoControl).find('a:eq('+ vars.currentSlide +')').addClass('active');
			}
